    ...
    $(document).ready(function(){
        if ($("#wmd-button-row").length >0){
            ...
            $('#wmd-button-row').append(
				...
				'<li class="wmd-button" id="wmd-weizhuang-button" style="" title="伪装">黑</li>'+
            ...
            $(document).on('click', '#wmd-button-button', function() {//按钮
                ...
                $('body').append(
                    ...
                    '<p><labe>样式</labe></p>'+
                    '<p><select id="co" style="width: 20%"><option value="light">白色</option><option value="info">蓝色</option><option value="dark">深色</option><option value="success">绿色</option><option value="black">黑色</option><option value="warning">黄色</option><option value="primary">紫色</option><option value="danger">红色</option></select>'+
					'<select id="ic" style="width: 55%"><option value="">按钮图标(选填)仅矩形支持</option><option value="glyphicon glyphicon-link">链接</option><option value="glyphicon glyphicon-download">下载</option><option value="glyphicon glyphicon-upload">上传</option><option value="glyphicon glyphicon-file">文件</option><option value="glyphicon glyphicon-heart">心形</option></select>'+
					'<select id="ty" style="width: 25%"><option value="">矩形</option><option value="round">椭圆形</option></select></p>'+
                    '</div>'+
                    '<form>'+
            ...
            $(document).on('click', '#wmd-text-button', function() {//标签
                ...
                $('body').append(
                    ...
                    '<p><b>插入代码框引用</b></p>'+
                    '<p><labe>样式</labe></p>'+
                    '<p><select id="type" style="width: 100%"><option value="share">资料灰@</option><option value="yellow">提示黄!</option><option value="red">警告红×</option><option value="blue">信息蓝i</option><option value="green">推荐绿√</option></select></p>'+
                    '</div>'+
                    '<form>'+
            ...
            $(document).on('click', '#wmd-code-button', function() {
            ...
            $(document).on('click', '#wmd-weizhuang-button', function() {

                textContent = '<span class="heimu" title="被你发现了！">伪装内容</span>';
...
//ICON
                var obj_ic = document.getElementById("ic"); //定位id
                var index_ic = obj_ic.selectedIndex; // 选中索引
                var icon = obj_ic.options[index_ic].value; // 选中值
//URL
                var url = $('.wmd-prompt-dialog input[name="url"]').val();
//输出格式
                //不同格式textContent = '[button color="' + color + '" icon="' + icon + '" type="' + type + '"][' + textContent + '](' + url + ')[/button]';
